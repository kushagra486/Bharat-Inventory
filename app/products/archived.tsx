...
import { getProducts, archiveProduct, deleteProduct } from '@/lib/db';
...
export default function ArchivedProductsScreen() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const data = await getProducts({ is_archived: true });
    setProducts(data);
    setLoading(false);
  }, []);
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={styles.card}>
            <TouchableOpacity style={styles.cardMain} onPress={() => router.push(`/products/${item.id}`)}>
              <Text style={styles.cardIcon}>{item.category?.icon ?? '📦'}</Text>
              <View style={{ flex: 1 }}>
                <Text style={styles.cardName} numberOfLines={1}>{item.name}</Text>
                <Text style={styles.cardMeta}>Expired {formatDate(item.expiry_date)}</Text>
              </View>
            </TouchableOpacity>
            <View style={styles.cardActions}>
              <TouchableOpacity style={styles.iconBtn} onPress={() => handleRestore(item.id)}>
                <Ionicons name="arrow-undo-outline" size={18} color={COLORS.accentCyan} />
              </TouchableOpacity>
        ...
        ListEmptyComponent={() => !loading ? (
          <View style={styles.empty}>
            ...
            <Text style={styles.emptySubtitle}>Products you archive will show up here</Text>
          </View>
        ) : null}
      />
    </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  iconBtn: {
    width: 36, height: 36, borderRadius: RADIUS.sm, backgroundColor: COLORS.bgElevated,
